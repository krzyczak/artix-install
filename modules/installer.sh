...
net_pkgs="dhcpcd-$MY_INIT avahi-$MY_INIT nss-mdns"

wifi_packages=""
if [ "$ENABLE_WIFI" = "y" ]; then
  wifi_packages="networkmanager networkmanager-openrc"
fi

pkgs="base base-devel $MY_INIT elogind-$MY_INIT efibootmgr grub openssh-$MY_INIT neovim less btop curl $git_pkgs $net_pkgs $wifi_packages"
...
KERNEL_PKG=(linux linux-headers linux-firmware)
basestrap /mnt "${KERNEL_PKG[@]}" mkinitcpio
